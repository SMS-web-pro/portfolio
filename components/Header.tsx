...
export default function Header() {
  ...
  const alternateLang = isFR ? 'en' : 'fr'
  const alternatePath = isFR ? '/' : '/fr/'

  useEffect(() => {
    setIsHomePage(pathname === '/' || pathname === '/fr/')
  }, [pathname])

  const navItems = [
    { label: t.header.services, href: isHomePage ? '#services' : (isFR ? '/fr/#services' : '/#services') },
    { label: t.header.projects, href: isHomePage ? '#projects' : (isFR ? '/fr/#projects' : '/#projects') },
    { label: t.header.expertise, href: isHomePage ? '#expertise' : (isFR ? '/fr/#expertise' : '/#expertise') },
    { label: t.header.contact, href: isHomePage ? '#contact' : (isFR ? '/fr/#contact' : '/#contact') },
  ]
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            >
              ...
              <span className="uppercase tracking-wide">{alternateLang}</span>
            </Link>
        ...
        {mobileMenuOpen && (
          <nav className="md:hidden pb-4 flex flex-col gap-3 animate-fade-in-down">
            ...
            <div className="flex gap-2 mt-2">
              <Link
                href={alternatePath}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors bg-[#f5f6f8] text-foreground hover:bg-[#e8f0f9]"
              >
                <span className="text-xs font-bold bg-[#0047ab]/10 text-[#0047ab] px-1.5 py-0.5 rounded">{alternateLang.toUpperCase()}</span> {alternateLang === 'fr' ? 'Français' : 'English'}
              </Link>
            </div>
