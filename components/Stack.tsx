...
export default function Stack() {
  ...
  return (
    <section id="expertise" className="relative overflow-hidden bg-secondary/70 py-20 text-foreground sm:py-28">
        ...
        {tracks.map((track, index) => (
          <div key={index} className="marquee-viewport marquee-mask overflow-hidden py-1" tabIndex={0} aria-label={language === 'fr' ? 'Défilement des technologies, mettre au focus pour suspendre' : 'Technology marquee, focus to pause'}>
            <div className={`marquee-track gap-3 px-2 ${index === 1 ? 'marquee-reverse' : ''}`}>
              {[...track, ...track].map((item, itemIndex) => <span key={`${item}-${itemIndex}`} aria-hidden={itemIndex >= track.length} className="inline-flex min-h-12 shrink-0 items-center rounded-full border border-primary/15 bg-background/75 shadow-sm px-5 font-mono text-sm text-foreground/85 backdrop-blur-sm transition-colors hover:border-accent/50 hover:bg-accent/10 hover:text-foreground">{item}</span>)}
            </div>
          </div>
        ))}
      </div>
      <div className="relative z-10 mx-auto mt-14 grid max-w-7xl grid-cols-2 gap-px overflow-hidden rounded-xl border border-primary/10 bg-primary/10 shadow-sm sm:grid-cols-4">
        {[
        ...
        ].map(([value, label]) => <div key={label} className="bg-background/90 p-5 backdrop-blur-sm sm:p-7"><p className="text-3xl font-bold text-primary sm:text-4xl">{value}</p><p className="mt-2 text-xs leading-relaxed text-muted-foreground sm:text-sm">{label}</p></div>)}
      </div>
    </section>
  )
}
