...
export default function Hero() {
  ...
  return (
    <section className="relative flex min-h-[760px] items-center overflow-hidden bg-secondary px-4 pb-16 pt-28 text-foreground sm:px-6 sm:pt-32 lg:min-h-screen lg:px-8">
      <video className="absolute inset-0 size-full object-cover opacity-20 mix-blend-multiply" autoPlay muted loop playsInline preload="metadata" aria-hidden="true">
        <source src="/gosite-hero.mp4" type="video/mp4" />
      </video>
      <div className="absolute inset-0 bg-gradient-to-r from-background via-background/90 to-secondary/70" />
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-12 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-4xl">
          <div className="mb-7 inline-flex items-center gap-3 rounded-full border border-primary/15 bg-background/80 px-4 py-2 font-mono text-xs uppercase tracking-[0.16em] text-primary shadow-sm backdrop-blur-md animate-fade-in-down">
            <span className="size-2 rounded-full bg-accent shadow-[0_0_12px_var(--accent)]" />
            {copy.eyebrow}
          </div>
          <h1 className="max-w-4xl text-balance text-4xl font-bold leading-[1.06] tracking-[-0.04em] text-foreground animate-fade-in-up sm:text-6xl lg:text-7xl xl:text-8xl">
            {copy.title}
          </h1>
          <p className="mt-7 max-w-2xl text-pretty text-base leading-relaxed text-foreground/75 animate-fade-in-up sm:text-lg lg:text-xl">
            {copy.subtitle}
          </p>
          ...
          <ul className="mt-10 flex flex-col gap-3 text-sm text-foreground/70 sm:flex-row sm:flex-wrap sm:gap-6">
        ...
        <div className="flex items-center justify-between gap-6 border-t border-background/20 pt-5 lg:w-64 lg:flex-col lg:items-start">
          <div><p className="font-mono text-xs uppercase tracking-[0.16em] text-foreground/50">GoSite</p><p className="mt-2 text-sm font-medium text-foreground">{copy.availability}</p></div>
          <a href="#about" aria-label={language === 'fr' ? 'Faire défiler vers la présentation' : 'Scroll to about'} className="flex size-11 items-center justify-center rounded-full border border-background/25 text-foreground transition-colors hover:bg-background hover:text-foreground"><ArrowDown aria-hidden="true" className="size-4 animate-bounce" /></a>
        </div>
      </div>
    </section>
  )
